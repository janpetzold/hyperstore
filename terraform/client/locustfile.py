...
class HyperstoreUserBehavior(SequentialTaskSet):
    ...
    def read_hyper(self):
        ...
        for _ in range(random.randint(5, 12)):
            with self.client.get("/api/hyper", headers={"Authorization": f"Bearer {self.read_access_token}"}, catch_response=True) as response:
                if response.status_code != 200:
                    response.failure("Could not read hyper")
            self.wait()
    ...
    def read_hyper_again(self):
        ...
        for _ in range(random.randint(2, 5)):
            with self.client.get("/api/hyper", headers={"Authorization": f"Bearer {self.read_access_token}"}, catch_response=True) as response:
                if response.status_code != 200:
                    response.failure("Could not read hyper (again)")
            self.wait()
    ...
    def post_endpoint_call_few_times(self):
        ...
        """Step 6: Reset stock to 500 hyper"""
        with self.client.post("/api/hyper", headers={"Authorization": f"Bearer {self.write_access_token}"}, json={"quantity": 500}, catch_response=True) as response:
            if response.status_code != 200:
